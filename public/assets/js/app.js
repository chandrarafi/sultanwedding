$(function () {
  ...
  // Inisialisasi PerfectScrollbar dengan pemeriksaan elemen
  ...
  $.ajaxSetup({
    beforeSend: function (xhr, settings) {
      // Add CSRF token only for POST, PUT, DELETE requests
      if (!/^(GET|HEAD|OPTIONS|TRACE)$/i.test(settings.type)) {
        // Try to get the token from the meta tag
        var csrfToken = $('meta[name="csrf_token"]').attr("content");
        var csrfHeader = $('meta[name="csrf_header"]').attr("content");

        if (csrfToken) {
          xhr.setRequestHeader(csrfHeader || "X-CSRF-TOKEN", csrfToken);
        }
      }
    },
  });

  // Fungsi untuk memperbarui token CSRF di meta tag
